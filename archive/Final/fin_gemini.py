...
class AlphaStockDataset(Dataset):
    def __init__(self, data_dir, markets=["nasdaq", "nyse", "sp500", "forbes2000"], 
        ...
        self.raw_prices = merged_close.values # <--- ADD THIS LINE (برای نگهداری قیمت‌های واقعی جهت رسم نمودار)
...
def plot_business_dashboard(history, save_dir):
    print("\n📊 Generating Realistic Business Dashboard...")
    fig = plt.figure(figsize=(16, 10))
    fig.canvas.manager.set_window_title('Alpha-MaTCHS Performance')
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, 'Alpha_MaTCHS_Dashboard.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"✅ Dashboard saved successfully at: {save_path}")

# ==============================================================================
# 5. NEW: 30-DAY PRICE & PREDICTION CHARTS
# ==============================================================================

def plot_prediction_samples(model, dataset, history, device, save_dir):
    print("\n📈 Generating 30-Day Price & Prediction Charts...")
    model.eval()
    
    # گرفتن آخرین داده‌های بازار (30 روز اخیر تا به امروز)
    last_idx = len(dataset) - 1
    past_tensor, target_reg, target_cls = dataset[last_idx]
    
    batch_x = past_tensor.unsqueeze(0).to(device)
    adj_matrix = dataset.relation_matrix.to(device)
    
    with torch.no_grad():
        pred_reg, pred_cls = model(batch_x, adj_matrix)
        # گرفتن احتمالات از شبکه
        pred_probs = torch.sigmoid(pred_cls)
    
    pred_returns = pred_reg.cpu().numpy()[0]
    pred_probs = pred_probs.cpu().numpy()[0]
    
    # پیدا کردن 4 سهمی که ربات بیشترین اطمینان را به صعود یا سقوطشان دارد
    confidence = np.abs(pred_probs - 0.5)
    top_4_indices = np.argsort(confidence)[-4:][::-1]
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 10))
    fig.suptitle('Alpha-MaTCHS: 30-Day History & Tomorrow\'s Prediction', fontsize=18, fontweight='bold')
    axes = axes.flatten()
    
    # محاسبه درصد سود کل استراتژی از روی تاریخچه
    total_profit_pct = (np.cumprod(1 + np.array(history['strategy_returns']))[-1] - 1) * 100
    
    for i, stock_idx in enumerate(top_4_indices):
        ax = axes[i]
        stock_name = dataset.stock_symbols[stock_idx]
        
        # استخراج قیمت‌های خام 30 روز گذشته
        end_day = last_idx + dataset.seq_len_past
        start_day = last_idx
        historical_prices = dataset.raw_prices[start_day:end_day, stock_idx]
        last_price = historical_prices[-1]
        
        # محاسبه قیمت پیش‌بینی شده برای فردا (تبدیل بازده لگاریتمی به قیمت واقعی)
        predicted_price = last_price * np.exp(pred_returns[stock_idx])
        
        days = np.arange(1, dataset.seq_len_past + 1)
        ax.plot(days, historical_prices, color='blue', linewidth=2.5, label='Past 30 Days (Real Price)')
        
        # رسم خط چین پیش‌بینی فردا
        color = 'green' if pred_probs[stock_idx] > 0.5 else 'red'
        ax.plot([dataset.seq_len_past, dataset.seq_len_past + 1], [last_price, predicted_price], 
                color=color, linestyle='--', marker='*', markersize=12, label='AI Prediction (Tomorrow)')
        
        direction = "UP 🟢" if pred_probs[stock_idx] > 0.5 else "DOWN 🔴"
        ax.set_title(f"{stock_name} | Pred: {direction} (Conf: {pred_probs[stock_idx]*100:.1f}%)", fontsize=13)
        ax.set_ylabel("Price (USD)")
        ax.set_xlabel("Days")
        ax.grid(True, alpha=0.3)
        ax.legend()
        
    # اضافه کردن نشانگر (Badge) سود کل در پایین تصویر
    fig.text(0.5, 0.02, f"🔥 Total Strategy Simulated Profit: +{total_profit_pct:.2f}% 🔥", 
    ...
    save_path = os.path.join(save_dir, 'Alpha_MaTCHS_Predictions.png')
    plt.savefig(save_path, dpi=300)
    print(f"✅ Prediction charts saved successfully at: {save_path}")
...
if __name__ == "__main__":
    ...
    save_dir = "/home/alien-hawk/Desktop/PEDC/Final" # <--- آدرس دقیق ذخیره عکس‌ها
    ...
    # 4. Generate Business Dashboards
    plot_business_dashboard(history, save_dir)
    plot_prediction_samples(model, dataset, history, device, save_dir)
    
    # Save Final Weights
    weights_path = os.path.join(save_dir, 'alpha_matches_weights.pth')
